function meanErr = RE_ProblemSet5Problem5()
...
for k = 2:(Nsim+1) % for k = 1:Nsim (2:Nsim+1 because Matlab is 1-based indexing)
    ...
    vk_prev = mvnrnd(zeros(2,1),Q)';
    personStates(:,k) = personStates(:,k-1) -theta * grad/norm(grad) + vk_prev;
    % Generate a measurement here: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [a,~] = altitudeAndGradient(personStates(1,k),personStates(2,k));
    wk = mvnrnd(zeros(1,1),R);
    measurements(k) = a + wk;
end
...
posteriorParticles(:,:,1) = repmat(s0,1,N) + mvnrnd(zeros(2,1),P0)';
