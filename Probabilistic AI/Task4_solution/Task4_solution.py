...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        net_out = self.logits_net(obs)
        pi = torch.distributions.Categorical(logits=net_out)
        return pi

    def _log_prob_from_distribution(self, pi: torch.distributions.Categorical, act):
        """
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # self.ptr > self.path_start_idx leading to [] slices. why does the below make sense?
        # in which cases this condition doesn't hold?
        self.ret_buf[self.ptr : self.path_start_idx] = np.cumsum(
            self.rew_buf[self.ptr : self.path_start_idx][::-1]
        )[::-1]

        # TODO: Implement TD residuals calculation.
        # Hint: use the discount_cumsum function
        # TODO: what about last_val?
        # TODO: how discount_cumsum really works?
        if isinstance(last_val, torch.Tensor):
            last_val = last_val.item()

        # as per the GAE formulation
        # self.tdres_buf[path_slice] = discount_cumsum(
        #     (rews)[:-1] - vals[:-1] + self.gamma * vals[1:],
        #     discount=self.gamma * self.lam,
        # )
        self.tdres_buf[path_slice] = (rews)[:-1] - vals[:-1] + self.gamma * vals[1:]

        # TODO: Implement discounted rewards-to-go calculation.
        # Hint: use the discount_cumsum function
        # this means take only rewards starting from timestep after an action was taken
        # which action?
        # this is vanilla discounted reward
...
class Agent:
    def __init__(self, env: LunarLander, activation=nn.Tanh):
        self.env = env
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        obs = data["obs"]
        act = data["act"]
        reward_to_go = data["ret"]
        tdres = data["tdres"]
        collected_logp_of_actions = data["logp"]

        pi, logp_of_actual_actions = agent.actor(obs, act)

        actor_loss = -torch.mean(tdres * logp_of_actual_actions)
        actor_loss.backward()
        ...
        for _ in range(100):
            ...
            value_est = agent.critic(obs)
            value_func_loss = torch.nn.functional.smooth_l1_loss(
                value_est, reward_to_go
            )
