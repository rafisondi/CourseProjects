...
def run_solution(
    ...
    if not combined_model:
        ...
        trainer1 = EnsembleTrainer(dataset_train=dataset_train)
        trainer2 = DropoutTrainer(dataset_train=dataset_train)
        trainer3 = SGLDTrainer(dataset_train=dataset_train)
        ...
        combined_trainer = CombinedTrainer(
            dataset_train=dataset_train, trainers=trainer_list
        )
        evaluate(combined_trainer, eval_loader, data_dir, output_dir)
        return combined_trainer


def calc_calibration_curve(
    predicted_probabilities: np.ndarray, labels: np.ndarray, num_bins=30
) -> dict:
    """
...
def combined_predict(
    data_loader: torch.utils.data.DataLoader, models_list: list
) -> np.ndarray:
    """
    ...
    probability_batches = []
    combined_trainer = models_list[0]

    for batch_x, _ in tqdm.tqdm(data_loader):
        # TODO: Combined model_3. Predict with your combined model
        current_probabilities = np.mean(
            [
                t.predict_probabilities(batch_x).detach().numpy()
                for t in combined_trainer.trainers
            ],
            axis=1,
        )
        probability_batches.append(current_probabilities)
...
class MNISTNet(nn.Module):
    def __init__(
        ...
        self.layer1 = nn.Linear(in_features, self.n_hidden)
        self.layer2 = nn.Linear(self.n_hidden, self.n_hidden)
        self.layer3 = nn.Linear(self.n_hidden, out_features)
...
class DropoutTrainer(Framework):
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        estimated_probability = torch.stack(
            [F.softmax(self.network(x), dim=1) for _ in range(num_sample)]
        ).mean(dim=0)
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train, *args, **kwargs):
        ...
        self.n_networks = 3
        n_hiddens = [np.random.randint(100, 150) for _ in range(self.n_networks)]
        self.EnsembleNetworks = [
            MNISTNet(
                in_features=28 * 28,
                out_features=10,
                dropout_at_eval=True,
                dropout_p=np.random.randint(30, 70) / 100*(n_hidden/np.max(n_hiddens))**1,
                n_hidden=n_hidden,
            )
        ...
        self.params = reduce(
            lambda a, b: a + b, [list(x.parameters()) for x in self.EnsembleNetworks]
        )
        ...
        self.train_subsets = [
            Subset(
                ...
                np.random.choice(
                    ...
                    replace=False,
                    size=len(dataset_train) // self.n_networks,
                ),
            )
        ...
        self.train_loaders = [
            ...
            for dataset_train_subset in self.train_subsets
        ]
        self.optimizers = [
            torch.optim.Adam(net.parameters(), lr=self.learning_rate)
            for net in (self.EnsembleNetworks)
        ]

    def train(self):

        for network_idx, network in enumerate(self.EnsembleNetworks):
            network.train()
            progress_bar = trange(self.num_epochs)
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(
                    self.train_loaders[network_idx]
                ):
                    # batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size,)
                    ...
                    #     F.log_softmax(current_logits, dim=1), batch_y, reduction="sum"
                    # )
                    loss = F.cross_entropy(current_logits, batch_y, reduction="mean")
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_probability = torch.stack(
            [
                F.softmax(self.EnsembleNetworks[i](x), dim=1)
...
class SGLDTrainer(Framework):
    def hinge_loss(self, ytrue, ypred):
        mask = np.ones(ypred.shape, dtype=bool)
        row_indx = np.arange(ytrue.shape[0])
        mask[row_indx, ytrue] = False
        losses = (
            1
            - ypred[~mask]
            + torch.max(ypred[mask].reshape(ytrue.shape[0], -1), dim=1)[0]
        )
        return torch.mean(torch.clip(losses, 0, None))
    ...
    def __init__(self, dataset_train, *args, **kwargs):
        ...
        # self.N = UnivariateGaussian(torch.tensor(0.0), torch.tensor(1.0))
        ...
        from smoothing import LabelSmoothingCrossEntropy

        self.loss = LabelSmoothingCrossEntropy(epsilon=0.1)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                # You need to calculate the loss based on the literature
                # loss = self.hinge_loss(ypred=current_logits, ytrue=batch_y)
                # loss = F.cross_entropy(current_logits, batch_y, reduction="mean")
                # loss = F.cross_entropy(current_logits, batch_y, reduction="mean")
                loss = self.loss(current_logits, batch_y)
                ...
                #         params += (
                #             np.sqrt(2 * self.learning_rate / self.inv_temperature)
                #             * self.N.sample()
                #         )
            ...
            if (
                num_iter > self.burn_in * self.sample_interval
                and num_iter % self.sample_interval == 0
            ):
                self.SGLDSequence.append(self.network)  # add model
                # self.scheduler.step()

            if len(self.SGLDSequence) == 10:
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_probability = torch.stack(
            [F.softmax(net(x), dim=1) for net in self.SGLDSequence]
        ).mean(dim=0)
...
class CombinedTrainer(Framework):
    def __init__(self, dataset_train, print_interval=50, *args, **kwargs):
        self.trainers = kwargs["trainers"]

    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        probability_samples = torch.stack(
            [t.predict_probabilities(x) for t in self.trainers], dim=0
        )
        estimated_probability = torch.mean(probability_samples, dim=0)
...
def evaluate(
    ...
    if EXTENDED_EVALUATION:
        ...
        reliability_diagram_datax = np.load(
            os.path.join(data_dir, "reliability_diagram_data_x.npy")
        )
        reliability_diagram_datay = np.load(
            os.path.join(data_dir, "reliability_diagram_data_y.npy")
        )
        for i in range(3):
