...
for i in progressbar(range(0,len(test_features.index),12) , "Reduce Patient Features for TESTING down to one line: ", 40):
    ...
    max = patient.max()-test_mean
    min = patient.min()-test_mean
    patient_red = pd.concat([max.abs(), min.abs()], axis=1).max(axis=1) - test_mean
    patient_red = pd.DataFrame([patient_red]).abs().astype(int)
    patient_red = patient_red.set_index("pid")
    test_features_reduced = pd.concat([test_features_reduced, patient_red.abs()],sort = False)
    #print(test_features_reduced)
...
for i in progressbar(range(0,len(train_features.index),12) , "Reduce Patient Features for TRAINING down to one line: ", 40):
    ...
    max = patient.max()-train_mean
    min = patient.min()-train_mean
    patient_red = pd.concat([max.abs(), min.abs()], axis=1).max(axis=1) - train_mean
    patient_red = pd.DataFrame([patient_red]).abs().astype(int)
    patient_red = patient_red.set_index("pid")
    train_features_reduced = pd.concat([train_features_reduced, patient_red.abs()],sort = False)
